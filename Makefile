...
load :
	...
	cat "source-data/NUTTAB_2010/NutriantFile_per_g.tab" | cut -d'	' -f2-4,6 | sort | uniq | grep -v '^"Nutrient ID"' > "data-for-import/Nutriants.tab"
	tail -n +2 < "source-data/NUTTAB_2010/NutriantFile_per_g.tab" | cut -d'	' -f1,2,5 > "data-for-import/NutriantFile_per_g.tab"
	tail -n +2 < "source-data/NUTTAB_2010/NutriantFile_per_ml.tab" | cut -d'	' -f1,2,5 > "data-for-import/NutriantFile_per_ml.tab"
